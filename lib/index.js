...
var TUNNEL_HEADER = 'x-whistle-policy';
...
function intercept(req, res) {
  ...
  return defer.then(function(port) {
    ...
    if (options.pathname === '/koa-whistle') {
      return Promise.resolve({
        statusCode: 302,
        headers: {
          location: '/koa-whistle/' + (options.search || '')
        }
      });
    }
    if (path.indexOf('/koa-whistle/') === 0) {
      options.path = path.replace('/koa-whistle', '');
      options.headers.host = 'local.wproxy.org';
    }
...
exports.getPortSync = function() {
  return process.env.KOA_WHISTLE_PORT || config.port;
};
...
exports.export = function(obj) {
  ['ready', 'getPort', 'getPortSync', 'setHttpsRequest', 'setHost', 'setProxy', 'setSocks',
  'createConnection', 'connect', 'startProxy'].forEach(function(name) {
