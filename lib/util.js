...
exports.HTTPS_HEADER = HTTPS_HEADER;
...
const normalizeOptions = (options) => {
  if (options instanceof Stream) {
    ...
    options.body = req;
    if (options.protocol === 'https:' || (req.socket && req.socket.encrypted)) {
      options.headers[HTTPS_HEADER] = 1;
    }
    delete options.protocol;
    options.uri = `http://${req.headers.host}${options.path}`;
    options.body = req;
    return options;
  }
...
const getRequest = (opts) => {
  const request = r.defaults({ proxy: `http://${opts.proxyHost}:${opts.proxyPort}` });
  return (options, cb) => {
