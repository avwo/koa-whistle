...
function handleRequest(req, res, options) {
  ...
  if (headers[HOST_HEADER]) {
    return res.end('/./ ' + decodeURIComponent(headers[HOST_HEADER]));
  }
  if (headers[PROXY_HEADER]) {
    return res.end('/./ proxy://' + decodeURIComponent(headers[PROXY_HEADER]));
  }
  if (headers[SOCKS_HEADER]) {
    return res.end('/./ socks://' + decodeURIComponent(headers[SOCKS_HEADER]));
  }
